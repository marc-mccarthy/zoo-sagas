...
function AddAnimal() {
    ...
    const submit = () => {
        ...
        dispatch({type: 'ADD_ANIMAL_SAGA', payload: { species, animalClass }});
        console.log(animalClass)
    }
	...
	return (
        <div>
            ...
            <div>Loading...</div>
                ...
                <form>
                    <input type="text" placeholder="Species" onChange={(e) => setSpecies(e.target.value)} value={species} />
                    <select onChange={(e) => setAnimalClass(e.target.value)}>
                        <option value="" disabled selected>Choose</option>
                        {animalClasses.map((ac => {
                            return <option key={ac.id} value={ac.id}>{ac.class_name}</option>
                        }))}
                    </select>
